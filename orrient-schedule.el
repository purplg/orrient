...
(orrient-schedule--add "Day and Night"
  :category 'core-tyria
  :events '(("Dawn"                              :offset ( 0 25) :frequency ( 2  0) :length ( 0  5) :waypoint nil)
...
(orrient-schedule--add "World Bosses"
  :category 'core-tyria
  :events '(("Admiral Taidha Covington"          :offset ( 0  0) :frequency ( 3  0) :length ( 0 15) :waypoint "[&BKgBAAA=]")
...
(orrient-schedule--add "Hard World Bosses"
  :category 'core-tyria
  :events '(("Tequatl the Sunless"               :offset ( 0  0) :frequency (24  0) :length ( 0 30) :waypoint "[&BNABAAA=]")
...
(orrient-schedule--add "Ley-Line Anomaly"
  :category 'core-tyria
  :events '(("Timberline Falls"                  :offset ( 0 20) :frequency ( 6  0) :length ( 0 20) :waypoint "[&BEwCAAA=]")
...
(orrient-schedule--add "PVP Tournaments"
 :category 'core-tyria
 :events '(("Balthazar's Brawl"                  :offset ( 0  0) :frequency (12  0) :length ( 1  0) :waypoint nil)
...
(orrient-schedule--add "Eye of the North"
 ...
 :events '(("Twisted Marionette"                 :offset ( 0  0) :frequency ( 2  0) :length ( 0 20) :waypoint "[&BAkMAAA=]")
...
(orrient-schedule--add "Scarlets Invasion"
  :category 'living-world-1
  :events '(("Defeat Scarlets minions"           :offset ( 1  0) :frequency ( 2  0) :length ( 0 15) :waypoint "[&BOQAAAA=]")))

(orrient-schedule--add "Dry Top"
  :category 'living-world-2
  :events '(("Crash Site"                        :offset ( 0  0) :frequency ( 1  0) :length ( 0 40) :waypoint nil)
...
(orrient-schedule--add "Verdant Brink"
 ...
 :events '(("Night: Night and the Enemy"         :offset ( 1 45) :frequency ( 2  0) :length ( 0 25) :waypoint nil)
...
(orrient-schedule--add "Auric Basin"
 :category 'heart-of-thorns
 :events '(("Challenges"                         :offset ( 0 45) :frequency ( 2  0) :length ( 0 15) :waypoint "[&BGwIAAA=]")
...
(orrient-schedule--add "Tangled Depths"
  :category 'heart-of-thorns
  :events '(("Prep"                              :offset ( 0 25) :frequency ( 2  0) :length ( 0  5) :waypoint nil)
...
(orrient-schedule--add "Dragons Stand"
  :category 'heart-of-thorns
  :events '(("Dragons Stand"                     :offset ( 0 90) :frequency ( 2  0) :length ( 2  0) :waypoint "[&BBAIAAA=]")))

(orrient-schedule--add "Lake Doric"
 :category 'living-world-3
 :events '(("Noran's Homestead"                  :offset ( 0 30) :frequency ( 2  0) :length ( 0 30) :waypoint "[&BK8JAAA=]")
...
(orrient-schedule--add "Crystal Oasis"
 ...
 :events '(("Rounds 1 to 3"                      :offset ( 0  5) :frequency ( 2  0) :length ( 0 10) :waypoint "[&BLsKAAA=]")
           ("Pinata/Reset"                       :offset ( 0 20) :frequency ( 2  0) :length ( 0 10) :waypoint "[&BLsKAAA=]")))

(orrient-schedule--add "Desert Highlands"
 :category 'path-of-fire
 :events '(("Buried Treasure"                    :offset ( 0  5) :frequency ( 2  0) :length ( 0 10) :waypoint "[&BGsKAAA=]")))

(orrient-schedule--add "Elon Riverlands"
 :category 'path-of-fire
 :events '(("The Path to Ascension: Augury Rock" :offset ( 1 30) :frequency ( 2  0) :length ( 0 25) :waypoint "[&BFMKAAA=]")
           ("Doppelganger"                       :offset ( 1 50) :frequency ( 2  0) :length ( 0 20) :waypoint "[&BFMKAAA=]")))

(orrient-schedule--add "The Desolation"
 :category 'path-of-fire
 :events '(("Junudu Rising"                      :offset ( 0 30) :frequency ( 1  0) :length ( 0 20) :waypoint "[&BMEKAAA=]")
           ("Maws of Torment"                    :offset ( 1  0) :frequency ( 2  0) :length ( 0 20) :waypoint "[&BKMKAAA=]")))

(orrient-schedule--add "Domain of Vabbi"
 :category 'path-of-fire
 :events '(("Forged with Fire"                   :offset ( 0  0) :frequency ( 2  0) :length ( 0 30) :waypoint "[&BO0KAAA=]")
...
(orrient-schedule--add "Domain of Istan"
 ...
 :events '(("Palawadan"                          :offset ( 1 45) :frequency ( 2  0) :length ( 0 30) :waypoint "[&BAkLAAA=]")))

(orrient-schedule--add "Jahai Bluffs"
 :category 'living-world-4
 :events '(("Escorts"                            :offset ( 1  0) :frequency ( 2  0) :length ( 0 15) :waypoint "[&BIMLAAA=]")
           ("Death-Branded Shatterer"            :offset ( 1 15) :frequency ( 2  0) :length ( 0 15) :waypoint "[&BJMLAAA=]")))

(orrient-schedule--add "Thunderhead Peaks"
 :category 'living-world-4
 :events '(("The Oil Floes"                      :offset ( 0 45) :frequency ( 2  0) :length ( 0 15) :waypoint "[&BKYLAAA=]")
...
(orrient-schedule--add "Grothmar Valley"
 ...
 :events '(("Effigy"                             :offset ( 0 10) :frequency ( 2  0) :length ( 0 15) :waypoint "[&BA4MAAA=]")
...
(orrient-schedule--add "Bjora Marches"
 :category 'icebrood-saga
 :events '(("Shards and Construct"               :offset ( 0  0) :frequency ( 2  0) :length ( 0  5) :waypoint "[&BCcMAAA=]")
...
(orrient-schedule--add "Dragonstorm"
 :category 'icebrood-saga
 :events '(("Dragonstorm"                        :offset ( 1  0) :frequency ( 2  0) :length ( 0 20) :waypoint "[&BAkMAAA=]")))
...
(orrient-schedule--add "Seitung Province"
 :category 'end-of-dragons
 :events '(("Aetherblade Assault"                :offset ( 1 30) :frequency ( 2  0) :length ( 0 30) :waypoint "[&BGUNAAA=]")))

(orrient-schedule--add "New Kaineng City"
 :category 'end-of-dragons
 :events '(("Kaineng Blackout"                   :offset ( 0  0) :frequency ( 2  0) :length ( 0 40) :waypoint "[&BBkNAAA=]")))

(orrient-schedule--add "The Echovald Wilds"
 :category 'end-of-dragons
 :events '(("Gang War"                           :offset ( 0 30) :frequency ( 2  0) :length ( 0 35) :waypoint "[&BMwMAAA=]")
           ("Aspenwood"                          :offset ( 1 40) :frequency ( 2  0) :length ( 0 20) :waypoint "[&BPkMAAA=]")))

(orrient-schedule--add "Dragons End"
  :category 'end-of-dragons
  :events '(("Jade Maw"                          :offset ( 0  5) :frequency ( 2  0) :length ( 0  8) :waypoint "[&BKIMAAA=]")
